...
class TwoPRec(object):
    ...
    def add_lickrates(self, t_prestim=2,
        ...
        _stim_rewdeliv_trrange = self.beh.rew.delivered[
            self.beh._stimrange.first:self.beh._stimrange.last]
        _licks_trrange = self.beh.lick.antic_raw[
            self.beh._stimrange.first:self.beh._stimrange.last]

        _tr_inds = {}
        _tr_inds['0'] = np.where(_stim_prob_trrange == 0)[0]
        _tr_inds['0.5'] = np.where(_stim_prob_trrange == 0.5)[0]
        _tr_inds['1'] = np.where(_stim_prob_trrange == 1)[0]

        _tr_inds['0.5_rew'] = np.where(np.logical_and(
            _stim_prob_trrange == 0.5, _stim_rewdeliv_trrange == True))[0]
        _tr_inds['0.5_norew'] = np.where(np.logical_and(
            _stim_prob_trrange == 0.5, _stim_rewdeliv_trrange == False))[0]
        _tr_inds['0.5_prelick'] = np.where(np.logical_and(
            _stim_prob_trrange == 0.5, _licks_trrange > 0))[0]
        _tr_inds['0.5_noprelick'] = np.where(np.logical_and(
            _stim_prob_trrange == 0.5, _licks_trrange == 0))[0]
        ...
        for trial_type in ['0', '0.5', '1', '0.5_rew', '0.5_norew',
            ...
            self.beh.lick.antic[trial_type] = np.zeros(
                _tr_inds[trial_type].shape[0])
            self.beh.lick.base[trial_type] = np.zeros(
                _tr_inds[trial_type].shape[0])

            for ind_trial, trial in enumerate(_tr_inds[trial_type]):
                if bl_norm is True:
                    _antic_licks = self.beh.lick.antic_raw[
                        self.beh._stimrange.first:
                        self.beh._stimrange.last][trial] \
    ...
    def plt_stim_aligned_avg(self,
        ...
        self.frame_avg.tr_inds['0'] = np.where(_stim_prob_trrange == 0)[0]
        self.frame_avg.tr_inds['0.5'] = np.where(_stim_prob_trrange == 0.5)[0]
        self.frame_avg.tr_inds['1'] = np.where(_stim_prob_trrange == 1)[0]

        self.frame_avg.tr_inds['0.5_rew'] = np.where(np.logical_and(
            _stim_prob_trrange == 0.5, _stim_rewdeliv_trrange == True))[0]
        self.frame_avg.tr_inds['0.5_norew'] = np.where(np.logical_and(
            _stim_prob_trrange == 0.5, _stim_rewdeliv_trrange == False))[0]
        self.frame_avg.tr_inds['0.5_prelick'] = np.where(np.logical_and(
            _stim_prob_trrange == 0.5, _licks_trrange > 0))[0]
        self.frame_avg.tr_inds['0.5_noprelick'] = np.where(np.logical_and(
            _stim_prob_trrange == 0.5, _licks_trrange == 0))[0]
        ...
        self.frame_avg.dff['0.5_rew'] = np.zeros((
            self.frame_avg.tr_inds['0.5_rew'].shape[0], n_frames_tot))
        self.frame_avg.dff['0.5_norew'] = np.zeros((
            self.frame_avg.tr_inds['0.5_norew'].shape[0], n_frames_tot))
        self.frame_avg.dff['0.5_prelick'] = np.zeros((
            self.frame_avg.tr_inds['0.5_prelick'].shape[0], n_frames_tot))
        self.frame_avg.dff['0.5_noprelick'] = np.zeros((
            self.frame_avg.tr_inds['0.5_noprelick'].shape[0], n_frames_tot))

        self.frame_avg.t = np.linspace(-1*t_pre, 2+t_post,
                                       n_frames_tot)
        ...
        for trial in range(self.beh._stimrange.first,
            ...
            if self.beh.stim.prob[trial] == 0:
                self.frame_avg.dff['0'][
                    self.frame_avg.tr_counts['0'], :] \
                    = _dff[0:n_frames_tot]
                self.frame_avg.tr_counts['0'] += 1
            elif self.beh.stim.prob[trial] == 0.5:
                self.frame_avg.dff['0.5'][
                    self.frame_avg.tr_counts['0.5'], :] \
                    = _dff[0:n_frames_tot]
                self.frame_avg.tr_counts['0.5'] += 1
            elif self.beh.stim.prob[trial] == 1:
                self.frame_avg.dff['1'][
                    self.frame_avg.tr_counts['1'], :] \
                    = _dff[0:n_frames_tot]
                self.frame_avg.tr_counts['1'] += 1

            if self.beh.stim.prob[trial] == 0.5 \
               and self.beh.rew.delivered[trial] == 1:
                self.frame_avg.dff['0.5_rew'][
                    self.frame_avg.tr_counts['0.5_rew'], :] \
                    = _dff[0:n_frames_tot]
                self.frame_avg.tr_counts['0.5_rew'] += 1
            elif self.beh.stim.prob[trial] == 0.5 \
                 and self.beh.rew.delivered[trial] == 0:
                self.frame_avg.dff['0.5_norew'][
                    self.frame_avg.tr_counts['0.5_norew'], :] \
                    = _dff[0:n_frames_tot]
                self.frame_avg.tr_counts['0.5_norew'] += 1

            if self.beh.stim.prob[trial] == 0.5 \
               and self.beh.lick.antic_raw[trial] > 0:
                self.frame_avg.dff['0.5_prelick'][
                    self.frame_avg.tr_counts['0.5_prelick'], :] \
                    = _dff[0:n_frames_tot]
                self.frame_avg.tr_counts['0.5_prelick'] += 1
            elif self.beh.stim.prob[trial] == 0.5 \
                 and self.beh.lick.antic_raw[trial] == 0:
                self.frame_avg.dff['0.5_noprelick'][
                    self.frame_avg.tr_counts['0.5_noprelick'], :] \
                    = _dff[0:n_frames_tot]
                self.frame_avg.tr_counts['0.5_noprelick'] += 1
    ...
    def plt_stim_aligned_sectors(self,
        ...
        self.plt_stim_aligned_avg(t_pre=t_pre, t_post=t_post, plot=False)
            ...
            for n_y in range(n_sectors):
                ...
                for tr_cond in ['0', '0.5', '1', '0.5_rew',
                    ...
                    self.sector.dff[n_x, n_y][tr_cond] = np.zeros((
                        self.sector.tr_inds[tr_cond].shape[0], n_frames_tot))
                ...
                for tr_cond in ['0', '0.5', '1']:
                    for tr in range(
                        ...
                        if resid_type == 'sector':
                            ...
                            _dff_mean = self.frame_avg.dff[tr_cond][tr, :]
