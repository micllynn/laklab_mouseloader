...
```bash
git clone https://github.com/mbfl/lak_exp.git
cd lak_exp
```
...
```python
import lak_exp as le
print(le.__file__)  # Should print package location
```
...
```python
from lak_exp.beh import BehDataSimpleLoad, BlockParser, TimelineParser
...
dset = le.DSetObj(path_reportopto='path_to_dataset.csv')

# Load a specific experiment (includes behavior, ephys, and alignment)
exp = le.ExpObj_ReportOpto(dset_obj=dset, dset_ind=5)
...
```python
exp = le.ExpObj_ValuePFC(dset_obj=dset, dset_ind=5)
```
...
## Analysis
- `lak_exp.analysis_fns` contains functions to load and plot entire datasets
comprised of many experiments.

```python
import lak_exp.analysis_fns as le_analysis
```

  - `le_analysis.ephys_all = get_ephys_all()` parses a dataset for all
  experiments containing recordings from
  a particular region
  - `le_analysis.plt_ephys_all_ordered(ephys_all)` plots a stimulus- or
